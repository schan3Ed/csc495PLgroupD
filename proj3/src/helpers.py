...
def guards():  return o.convert(
    {k: v for k, v in inspect.getmembers(helpers, inspect.isfunction) if k.startswith('g__')})


def actions(): return o.convert(
    {k: v for k, v in inspect.getmembers(helpers, inspect.isfunction) if k.startswith('a__')})


def shared():  return o.convert(
    {k: v for k, v in inspect.getmembers(helpers, inspect.isfunction) if k.startswith('s__')})
...
def getArgs(text, fString):
    ...
    if preOrderTraversal:
        regstr = "((?:(?!%s).)*)%s" % (
        regstr[4:regstr.rindex("(.*)")], regstr[4:])  # dont allow identical commands in the first parameter
...
# #################################################
def g__p09__for_every_player_X(
        args):  # similar to the action version of this function, but only returns true if the evaluated guard returns true for all players
    x1 = args[0]
...
def g__p09__for_any_player_X(
        args):  # similar to for_every_player guard, but returns true if at least 1 guards evaluate to true
    x1 = args[0]
    playerGuards = [re.sub(r"\bplayer\b", player, x1) for player in the.script.players]
    playerGuards = [buildGuard(guard) for guard in playerGuards]
...
def a__p09__for_every_player_X(
        args):  # builds and calls an action to perform once for every player by calling buildAction and providing one version of the text each time, but with the word 'player' replaced with player names like 'player1' 'player2' etc
    x1 = args[0]
    playerActions = [re.sub(r"\bplayer\b", player, x1) for player in the.script.players]
    playerActions = [buildAction(action) for action in playerActions]
...
def a__p06__X_draws_from_X_into_X_or_plays_from_X_into_X_where_X(args):
    def fun(args=args):
        ...
        isTrue = lambda x: g__p05__X_is_true([x])()
        play = lambda: choose(['draw'] + getExpr(x4).get(), autoplay=the.autoplay)
...
def a__p05__X_draws_from_X_into_X_or_plays_from_X_into_X(args):
    def fun(args=args):
        x1, x2, x3, x4, x5 = args
        choice = choose(['draw'] + getExpr(x4).get(), autoplay=the.autoplay)
        if 'draw' in choice:
            getExpr(x3).get().append(getExpr(x2).get().pop())
        else:
...
def a__p04__X_plays_from_X_into_X_where_X(args):
    def fun(args=args):
        x1, x2, x3, x4 = args
        isTrue = lambda x: g__p05__X_is_true([x])()
        play = lambda: choose(getExpr(x2).get(), autoplay=the.autoplay)
        backupEnvironment = copy.deepcopy(the.load)
        play()
        while not isTrue(x4):
...
def a__p00__transfer_X_cards_from_X_into_X(args):
    def fun(args=args):
        x1, x2, x3 = args
        x1 = getExpr(x1)
        x2 = getExpr(x2)
